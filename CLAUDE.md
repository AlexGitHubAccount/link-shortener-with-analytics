...
- **Модули**: `prisma` (глобальный), `health`, `auth` (Google OAuth **+ email/password**, JWT с server-side отзывом токена), `users`, `links`, `redirect`, `analytics`
...
- Модели: User (`googleId` nullable + `passwordHash` nullable — юзер приходит ЛИБО через Google, ЛИБО через email/password; сервис-слой гарантирует, что хотя бы одно не null. Связывание обоих способов на один аккаунт — **отложенная фича**, см. ниже), Link, Click (с enum'ом Click.DeviceType), RevokedToken
...
`POST /auth/register` (201) и `POST /auth/login` (200) — публичные (без `JwtAuthGuard`), оба под `@UseGuards(ThrottlerGuard)` + жёсткий `@Throttle` (register 10/60с, login 5/60с — точка брутфорса). Возвращают `{ token }` тем же payload'ом (`sub`/`email`/свежий `jti`), что и Google-callback — общий приватный хелпер `AuthService.issueToken`. Хеширование — `@node-rs/argon2` (argon2id, дефолтные параметры OWASP; prebuilt-бинарники, без node-gyp — не ломает CI/Docker). `login` возвращает **единый generic `401 "Invalid email or password"`** для трёх случаев (неверный пароль / несуществующий email / Google-only аккаунт без пароля) и всегда прогоняет argon2-операцию против dummy-хеша — иначе быстрый 401 на неизвестный email против медленного на известный был бы user-enumeration оракулом по таймингу. `getDummyHash()`: лениво инициализируется при первом входе (`dummyHashPromise ??= hash(...)`), не на уровне модуля (сбой argon2 при старте иначе клал бы весь API unhandled-rejection'ом, а не только вход), и **сбрасывает мемо при reject** (закешированный упавший промис заблокировал бы все последующие входы по неизвестному email до перезапуска). В `login` вся цепочка неизвестного-email пути (`getDummyHash().then(verify).catch(() => false)`) под **одним** catch — зеркально ветке реального пароля (`verify(...).catch(() => false)`): любая ветка может уронить свою argon2-операцию, и в обоих случаях отдаётся один generic 401, никогда не 500 (асимметрия кодов сама была бы оракулом). `verify()` обёрнут в `.catch(() => false)` — битый хеш в БД тоже даёт 401. Гонка при регистрации (два одновременных сабмита проходят pre-check «email свободен») ловится по Prisma `P2002` в `createPasswordUser` → тот же `409`, не `500`.

**Связывание Google + password на один аккаунт — НАМЕРЕННО не поддерживается** (отложенная фича). `findOrCreateByGoogleProfile` ищет строго по `googleId` (никогда по email) → нашёл: обновляет `displayName`/`avatarUrl` **и `email`** (нормализованный — Google здесь identity-провайдер строки, смена email на его стороне должна доехать, не-каноничная строка самолечится; коллизия обновлённого email с password-аккаунтом → `ConflictException`, как на `create`) → не нашёл: `create`. Если `create` падает на `P2002` — код НЕ доверяет `error.meta.target` (его форма варьируется между версиями Prisma/драйверами: колонка `["email"]` vs имя constraint'а `["User_email_key"]`; на нашей связке Prisma 6.19.3 + node-postgres — колонка, проверено): вместо этого перечитывает по `googleId` → нашёл ⇒ гонка двух одновременных Google-входов, возвращает строку → не нашёл ⇒ этим email владеет password-аккаунт ⇒ `ConflictException`, `googleAuthCallback` ловит её и редиректит на `/login?error=account_exists` (фронт показывает «войдите паролем»). Предыдущая версия этой фичи авто-связывала по совпадению email — это **account pre-hijacking**: атакующий регистрирует `victim@email` с известным ему паролем ДО первого входа жертвы, при Google-входе жертвы аккаунты сливаются, `passwordHash` атакующего остаётся рабочим. Ревью (`oncall-qa` + `oncall-devsecops` — оба независимо; затем `/code-review high` добавил нормализацию email и устойчивость к форме `P2002`) поймало это до push. Настоящее связывание требует email-верификации ИЛИ повторного ввода пароля при линковке — этого в проекте пока нет. Хелпер `common/prisma-errors.ts` `isUniqueConstraintViolation(error, field?)` — единый для `links`/`users`/`auth` (раньше был дубль в `links.service.ts`).

**Известные ограничения rate limiting** (унаследованы от `ThrottlerModule`, см. `app.module.ts`): счётчики in-memory, per-process — при >1 копии API эффективный лимит = N×настроенный (нужен общий стор, напр. Redis); ключ — per-IP, не per-account — распределённый брутфорс с многих IP не режется; per-account lockout нет. `@Throttle` 5/60с на login + argon2id (~40мс/проверка) закрывают одиночный источник; распределённая атака — принятый gap до реального деплоя.

Frontend: `/login` — сегментированный переключатель «Sign in / Create account» + Google под разделителем; общий хелпер `features/auth/completeLogin.ts` (token → `/auth/me` → стор → `navigate('/')`) используют все три пути входа (Google-callback, register, login). `?error=account_exists` в URL → алерт «войдите паролем» показывается на вкладке `login` (по умолчанию активна); переключение на «Create account» снимает алерт насовсем, переключатель не блокируется; параметр вычищается из URL. Email везде нормализуется (`trim().toLowerCase()`) — в `UsersService` (чокпоинт всех трёх путей, т.к. Google-email не проходит через DTO), плюс `@Transform` в DTO и `.trim().toLowerCase().pipe(z.email())` в zod-схемах; миграция `normalize_existing_emails` (с guard-блоком против коллизий) привела существующие строки. Без этого `John@X` при регистрации и `john@x` при входе — разные строки для Postgres (регистрозависимое сравнение) → юзер заблокирован в своём аккаунте, и коллизия email password-строки с Google-входом проскакивала бы мимо unique-индекса.
...
**Semgrep не в `.mcp.json`** — его конфиг требует абсолютный host-путь текущего чекаута в `docker run -v` (у каждого разработчика свой), а официальный путь `semgrep-mcp` через `uvx`/`pip` в этом окружении недоступен (см. Troubleshooting ниже). Semgrep здесь никем не запускается автоматически — CI гоняет `pnpm audit` (supply-chain), а разовый скан кода можно сделать вручную: `timeout 120 docker run --rm -v "$(pwd):/src" semgrep/semgrep semgrep scan --config=auto /src/apps/api/src /src/apps/web/src`. MCP-сервер Semgrep нужен только для интерактивных `mcp__semgrep__*` инструментов в чате — если хотите их, добавить в `.mcp.json` локально:
```json
"semgrep": {
  "type": "stdio",
  "command": "docker",
...
```
auth/             # JWT & Google OAuth & email/password (auth.service.ts), guards, декораторы
...
Владение (Agent Teams): юнит/компонентные тесты пишут сами разработчики (`core-backend` —
`*.spec.ts`, `core-frontend` — `*.test.tsx`), держат порог 80%. Сквозной E2E-набор
(`apps/e2e/**`) и ревью изменений — `oncall-qa`. Подробнее — `.claude/README.md`.
...
- **E2E**: Playwright, отдельный workspace `apps/e2e/` (не `apps/web/e2e/` — намеренно изолирован от конфига Vitest)
  ...
  - **a11y**: `@axe-core/playwright` на каждой ключевой странице (`/login`, Dashboard, LinkDetail/analytics) в `full-flow.spec.ts` — объективный, детерминированный скан WCAG2A/AA, только `serious`/`critical` impact (moderate/minor слишком часто спорные — например color-contrast на декоративных элементах — сделали бы проверку хрупкой). Дополняет, не заменяет ручное LLM-ревью (`oncall-qa` / `/code-review`) — объективная rule-based проверка как второе мнение.
...
- **`uvx`/`pip` недоступны**: в этом окружении нет Python-тулинга для пакетного менеджера, поэтому `semgrep-mcp` (изначально планировался как MCP-сервер) нельзя поставить через `uvx`. Если нужен разовый скан кода — официальный Docker-образ напрямую: `timeout 120 docker run --rm -v "$(pwd):/src" semgrep/semgrep semgrep scan --config=auto /src/apps/api/src /src/apps/web/src`.
...
**Релиз `v1.1.0`** (2026-09-03) — тег + GitHub Release, `main` зелёный в CI (`de37e2c`). MINOR: email/password-аутентификация рядом с Google OAuth (`POST /auth/register` + `/auth/login`, argon2id, rate-limited; переписанная `/login`). Обратно совместимо, миграции expand-only. Первая фича, целиком проведённая командой Agent Teams (`/feature`) + 3 раунда ревью (Шаг 4 + `/code-review high` + `/push-gate`). Секция `## [1.1.0]` `CHANGELOG.md`. Предыдущие: `v1.0.1` (2026-09-02, переработка команды Agent Teams + доки), `v1.0.0` (2026-09-01, весь продукт: 8 этапов + Tier 0).

**Известные ограничения:**
- **CD не настроен** — реального деплоя нет; выбор платформы (Fly.io/Railway/VPS) и разворачивание — задача `oncall-devsecops`, когда платформа выбрана. CI пока только проверяет. Поэтому и на GitHub Pages проект не посмотреть: Pages раздаёт статику, а `apps/api` (NestJS + PostgreSQL) нужен рантайм.
- **Google OAuth-код готов и проверен end-to-end** (2026-09-02, на живом бэкенде — см. `CHANGELOG.md`). Своя пара кредов Google Cloud нужна каждому развёртыванию и свежему клону: секреты не в репозитории (`.gitignore` + `permissions.deny`) — это природа OAuth, не пробел в реализации. Локальная настройка (`apps/api/.env` + redirect URI в Google Console) выполнена.
...
**Политика синхронизации документации**: на чистом прогоне `push-gate` синхронизирует затронутую документацию (этот файл, `CHANGELOG.md`, `README.md`, если поменялась установка) как часть подготовки push — не отдельный ручной шаг. См. `.claude/skills/push-gate/SKILL.md`, Шаг 4.

**Гейт перед push** (`.claude/hooks/push-gate.sh` + skill `push-gate`): pre-push hook блокирует любой `git push` без одноразовой расписки. `/push-gate` делает **один полный проход детерминированных проверок** по коммитам, которых ещё нет на remote — `pnpm lint`, affected `type-check`/`test:cov` (порог покрытия 80%, как в CI)/`build` (turbo `--filter="...[<base>]"`), скан секретов — плюс advisory-ревью `oncall-qa` (и `oncall-devsecops` в режиме ревью, если диапазон трогает security-поверхность: `auth/`, `common/guards\|decorators/`, `main.ts`, `redirect/`, FE `features/auth/`, `auth.store.ts`, `api-client.ts` — ИЛИ инфра-поверхность: `.github/workflows/`, `Dockerfile*`, `turbo.json`, `docker-compose.yml`, `nginx.conf.template`, `.dockerignore`, `pnpm-workspace.yaml`, `pnpm-lock.yaml`, любой `package.json`, `config/**`). Проверки идут до конца, собирают ВСЁ и отдаются человеку с подробным объяснением; **гейт сам код не правит и не коммитит** (прежний fixer-цикл на 15 итераций удалён — зависал и осциллировал, см. `CHANGELOG.md` от 2026-08-31). Push проходит только когда все детерминированные проверки зелёные И нет находок `severity: high` от ревьюеров. `git commit` — отдельный лёгкий гейт того же хука: только `pnpm lint`, без AI (коммитить, `--amend`, `rebase` можно свободно). Полная регрессия (весь тест-сьют, E2E, build) в `push-gate` не входит — это работа CI (`.github/workflows/ci.yml`).

**Команда разработки (Agent Teams)**: фичи монолита разрабатываются через `/feature <описание>` — плейбук ведущего (tech-lead = основная сессия). Флаг `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` в `.claude/settings.json` включает экспериментальные Agent Teams. Состав — **1 + 4**, как в современной продуктовой команде: у каждого файла и задачи ровно один владелец (полная карта — `.claude/skills/feature/SKILL.md`, «Полная карта ответственности»). Имя агента = частота: `core-*` (постоянное ядро, тиммейты почти на каждой фиче), `oncall-*` (по запросу — тиммейтами только когда фича задела их зону, плюс ревьюеры без имени всегда). **Лид = архитектор и координатор** — принимает архитектурные решения (контракт, модель данных, security-постура, семантика ошибок — Шаг 1.2), разбивает и координирует; **кода и инфры не пишет**, владеет только документацией и запуском гейта. Тиммейты (спавнятся только задетые фичей): `core-backend` («be») — `apps/api/src/**` бизнес-логика + свои `*.spec.ts` + `prisma/**`; `core-frontend` («fe») — `apps/web/src/**` + свои `*.test.tsx` («you build it, you test it», порог 80%); `oncall-qa` («qa») — `apps/e2e/**`, только под новый пользовательский флоу; `oncall-devsecops` («ops») — **вся инфра/CI/Docker/`turbo.json`/`main.ts`/`config/`/env-переменные/зависимости/релиз/деплой миграций** (забрали у лида). `oncall-qa` и `oncall-devsecops` — по две шляпы: тиммейт + субагент **без имени** в режиме ревью (Шаг 4 / push-gate), последний делает только задетый из security/devops проходов. `code-reviewer`+`test-engineer` слиты в `oncall-qa`; `devsecops-reviewer` вырос в `oncall-devsecops`. Для особо чувствительного диапазона (auth) — `/code-review high`/`ultra`. Файла конфигурации команды нет — `~/.claude/teams/session-*/` генерируется автоматически. Подробнее — `.claude/README.md`, `.claude/skills/feature/SKILL.md`.
