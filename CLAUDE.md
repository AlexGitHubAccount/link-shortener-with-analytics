...
**Команда разработки (Agent Teams)**: фичи монолита разрабатываются через `/feature <описание>` — плейбук ведущего (tech-lead = основная сессия). Флаг `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` в `.claude/settings.json` включает экспериментальные Agent Teams. Состав — **1 + 2 + 2**: **лид = архитектор** (решения о контракте, модели данных, миграциях, security-постуре, инфре — `/feature` Шаг 1.2, отдельного агента-архитектора нет и не должно быть; инфра-файлы `.github/`/`Dockerfile*`/`turbo.json` правит лид напрямую) + два тиммейта `backend-dev` / `frontend-dev` (роли по границам владения файлами — двое не трогают один файл; **каждый пишет свои тесты** и держит порог покрытия 80% на своём слое, отдельного `test-engineer` нет; e2e закрепляется явно за одним) + два ревьюера-субагента **без имени** после готовности фичи: `code-reviewer` всегда, `devsecops-reviewer` если задета security- ИЛИ инфра-поверхность (совмещает обе роли, два внутренних прохода, делает только задетый). Для особо чувствительного диапазона (auth) — `/code-review high`/`ultra`. Файла конфигурации команды нет — `~/.claude/teams/session-*/` генерируется автоматически. Подробнее — `.claude/README.md`, `.claude/skills/feature/SKILL.md`.
