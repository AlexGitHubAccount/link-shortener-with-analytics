...
**Гейт перед push** (`.claude/hooks/push-gate.sh` + skill `push-gate`): pre-push hook блокирует любой `git push` без одноразовой расписки. `/push-gate` делает **один полный проход детерминированных проверок** по коммитам, которых ещё нет на remote — `pnpm lint`, affected `type-check`/`test:cov` (порог покрытия 80%, как в CI)/`build` (turbo `--filter="...[<base>]"`), скан секретов — плюс advisory-проход `code-reviewer` (и `security-reviewer`, если диапазон трогает auth-поверхность: `auth/`, `common/guards\|decorators/`, `main.ts`, `redirect/`, FE `features/auth/`, `auth.store.ts`, `api-client.ts`). Проверки идут до конца, собирают ВСЁ и отдаются человеку с подробным объяснением; **гейт сам код не правит и не коммитит** (прежний fixer-цикл на 15 итераций удалён — зависал и осциллировал, см. `CHANGELOG.md` от 2026-08-31). Push проходит только когда все детерминированные проверки зелёные И нет находок `severity: high` от ревьюеров. `git commit` — отдельный лёгкий гейт того же хука: только `pnpm lint`, без AI (коммитить, `--amend`, `rebase` можно свободно). Полная регрессия (весь тест-сьют, E2E, build) в `push-gate` не входит — это работа CI (`.github/workflows/ci.yml`).

**Команда разработки (Agent Teams)**: фичи монолита разрабатываются через `/feature <описание>` — плейбук ведущего (tech-lead = основная сессия). Флаг `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` в `.claude/settings.json` включает экспериментальные Agent Teams. Состав: **лид = архитектор** (решения о контракте, модели данных, миграциях, security-постуре — `/feature` Шаг 1.2, отдельного агента-архитектора нет и не должно быть) + тиммейты `backend-dev` / `frontend-dev` / `test-engineer` (роли по границам владения файлами — двое не трогают один файл) + ревьюеры-субагенты **без имени** после готовности фичи: `code-reviewer` всегда, `security-reviewer` если задета auth-поверхность. DevOps отдельной ролью нет — инфра-файлы трогаются редко, это лид напрямую. Для особо чувствительного диапазона (auth) — `/code-review high`/`ultra`. Файла конфигурации команды нет — `~/.claude/teams/session-*/` генерируется автоматически. Подробнее — `.claude/README.md`, `.claude/skills/feature/SKILL.md`.
