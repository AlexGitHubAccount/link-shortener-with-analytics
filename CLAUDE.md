...
**Гейт перед push** (`.claude/hooks/push-gate.sh` + skill `push-gate`): pre-push hook блокирует любой `git push` без одноразовой расписки. `/push-gate` делает **один полный проход детерминированных проверок** по коммитам, которых ещё нет на remote — `pnpm lint`, affected `type-check`/`test:cov` (порог покрытия 80%, как в CI)/`build` (turbo `--filter="...[<base>]"`), скан секретов — плюс advisory-ревью `qa-engineer` (и `devsecops-reviewer`, если диапазон трогает security-поверхность: `auth/`, `common/guards\|decorators/`, `main.ts`, `redirect/`, FE `features/auth/`, `auth.store.ts`, `api-client.ts` — ИЛИ инфра-поверхность: `.github/workflows/`, `Dockerfile*`, `turbo.json`, `docker-compose.yml`, `nginx.conf.template`, `.dockerignore`, `pnpm-workspace.yaml`, `pnpm-lock.yaml`, любой `package.json`, `config/env.validation.ts`). Проверки идут до конца, собирают ВСЁ и отдаются человеку с подробным объяснением; **гейт сам код не правит и не коммитит** (прежний fixer-цикл на 15 итераций удалён — зависал и осциллировал, см. `CHANGELOG.md` от 2026-08-31). Push проходит только когда все детерминированные проверки зелёные И нет находок `severity: high` от ревьюеров. `git commit` — отдельный лёгкий гейт того же хука: только `pnpm lint`, без AI (коммитить, `--amend`, `rebase` можно свободно). Полная регрессия (весь тест-сьют, E2E, build) в `push-gate` не входит — это работа CI (`.github/workflows/ci.yml`).

**Команда разработки (Agent Teams)**: фичи монолита разрабатываются через `/feature <описание>` — плейбук ведущего (tech-lead = основная сессия). Флаг `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` в `.claude/settings.json` включает экспериментальные Agent Teams. Состав — **1 + 2 + 2**, как в современной продуктовой команде: **лид = архитектор** (решения о контракте, модели данных, миграциях, security-постуре, инфре — `/feature` Шаг 1.2, отдельного агента-архитектора нет и не должно быть; инфра-файлы `.github/`/`Dockerfile*`/`turbo.json` правит лид напрямую) + два тиммейта-разработчика `backend-dev` / `frontend-dev` (роли по границам владения файлами — двое не трогают один файл; каждый владеет своим кодом **вместе со своими юнит/компонентными тестами**, «you build it, you test it», порог 80%) + `qa-engineer` — инженер по качеству с двумя шляпами: субагент-ревью **без имени** на каждом диапазоне (корректность, конвенции, безопасность, адекватность тестов) + тиммейт `qa`, владеющий `apps/e2e/**`, спавнится только когда фича добавляет новый пользовательский флоу + `devsecops-reviewer` — субагент **без имени** по условию (security ИЛИ devops-проход, делает только задетый). `code-reviewer` и `test-engineer` объединены в `qa-engineer`. Для особо чувствительного диапазона (auth) — `/code-review high`/`ultra`. Файла конфигурации команды нет — `~/.claude/teams/session-*/` генерируется автоматически. Подробнее — `.claude/README.md`, `.claude/skills/feature/SKILL.md`.
