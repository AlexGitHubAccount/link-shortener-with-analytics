...
**Гейт перед push** (`.claude/hooks/push-gate.sh` + skill `push-gate`): pre-push hook блокирует любой `git push` без одноразовой расписки. `/push-gate` делает **один полный проход детерминированных проверок** по коммитам, которых ещё нет на remote — `pnpm lint`, affected `type-check`/`test`/`build` (turbo `--filter="...[<base>]"`), скан секретов — плюс один advisory-проход `code-reviewer`. Проверки идут до конца, собирают ВСЁ и отдаются человеку с подробным объяснением; **гейт сам код не правит и не коммитит** (прежний fixer-цикл на 15 итераций удалён — зависал и осциллировал, см. `CHANGELOG.md` от 2026-08-31). Push проходит только когда все детерминированные проверки зелёные. `git commit` — отдельный лёгкий гейт того же хука: только `pnpm lint`, без AI (коммитить, `--amend`, `rebase` можно свободно). Полная регрессия (весь тест-сьют, E2E, build) в `push-gate` не входит — это работа CI (`.github/workflows/ci.yml`).

**Команда разработки (Agent Teams)**: фичи монолита разрабатываются через `/feature <описание>` — плейбук ведущего (tech-lead = основная сессия). Флаг `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` в `.claude/settings.json` включает экспериментальные Agent Teams. Ведущий разбивает фичу на задачи с закреплением владения файлами (двое тиммейтов не трогают один файл), спавнит тиммейтов `backend-dev` / `frontend-dev` / `test-engineer` (субагент-определения в `.claude/agents/*-dev.md`, они же работают как обычные субагенты), координирует, затем запускает `code-reviewer` субагентом без имени и `/push-gate`. Для особо чувствительного диапазона (auth) — встроенный `/code-review high`/`ultra`. Файла конфигурации команды нет — `~/.claude/teams/session-*/` генерируется автоматически. Подробнее — `.claude/README.md`, `.claude/skills/feature/SKILL.md`.
