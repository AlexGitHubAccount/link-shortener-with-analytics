...
Владение (Agent Teams): юнит/компонентные тесты пишут сами разработчики (`backend-dev` —
`*.spec.ts`, `frontend-dev` — `*.test.tsx`), держат порог 80%. Сквозной E2E-набор
(`apps/e2e/**`) и ревью изменений — `qa-engineer`. Подробнее — `.claude/README.md`.
