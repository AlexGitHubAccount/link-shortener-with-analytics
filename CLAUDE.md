...
**Релиз `v1.0.1`** (2026-09-02) — тег + GitHub Release, `main` зелёный в CI. Только `.claude/**` (переработка команды Agent Teams) + документация, рантайм-код не менялся — что и почему в секции `## [1.0.1]` `CHANGELOG.md`. Предыдущий `v1.0.0` (2026-09-01) — весь продукт: 8 этапов + Tier 0 production-подготовка, секция `## [1.0.0]`.
