...
**Статус**: 🟡 Опубликовано, CI/CD зелёные, ждёт `/stage-review 7` (2026-08-21)
...
Каждая находка воспроизведена и исправлена именно через настоящий `git clone` в чистую директорию (а не полагаясь на локальное рабочее дерево, где закэшированные артефакты маскировали проблему), затем подтверждена реальным зелёным прогоном на GitHub Actions. Итоговый прогон (#4): **CI ✅, E2E ✅**, оба зелёные.
