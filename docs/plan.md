...
2. **Заход 2 (широкий скоуп)** — весь накопленный код проекта целиком (`apps/api/src apps/web/src packages/`), `code-review` на уровне `medium` + `security-review`. Ловит интеграционные проблемы между этапами, которые не видны в узком диффе.
