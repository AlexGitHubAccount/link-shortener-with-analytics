...
Прежде чем отмечать этап ✅ в таблице выше, он проходит **двухзаходное ревью** через skill `/stage-review N` (`.claude/skills/stage-review/SKILL.md`):

1. **Заход 1 (узкий скоуп)** — только диф нового этапа (`git diff stage-(N-1)-done..HEAD`), `code-review` на уровне `medium` + `security-review`. Находки чинятся, заход повторяется до чистого результата.
2. **Заход 2 (широкий скоуп)** — весь накопленный код проекта целиком (`apps/api/src apps/web/src packages/`), только `code-review` на уровне `medium`. Ловит интеграционные проблемы между этапами, которые не видны в узком диффе.

Уровень `medium` выбран сознательно ради экономии токенов: `code-review` возвращает только высокоуверенные находки (в отличие от `high`/`xhigh`/`max`, которые дают более широкое покрытие ценой более частых «может быть, а может и нет»). Для этапов с повышенными рисками (например, Этап 4 — авторизация) можно вручную поднять уровень или запустить `/code-review ultra`.

**`security-review` есть только в Заходе 1, не в Заходе 2** — практический вывод из первого реального прогона на Этапе 1: этот skill не принимает путь как скоуп, он всегда сам строит `git diff origin/HEAD...HEAD`. Поскольку `origin` (GitHub-remote) в проекте официально нет до Этапа 7, `stage-review` создаёт **локальную** git-ссылку `refs/remotes/origin/HEAD` (не настоящий remote, ничего никуда не пушится) и указывает её на `stage-(N-1)-done` перед каждым Заходом 1 — так диф остаётся маленьким и релевантным. Если тянуть этот же диф до самого начала проекта для Захода 2, в промпт попадают сотни килобайт сгенерированных файлов (`pnpm-lock.yaml`, SQL-миграции) — дорого и бессмысленно. Security-покрытие всего кода при этом не теряется: каждый этап получает свой `security-review`, когда его код ещё новый.

Только после чистого Захода 2 ставится git-тег `stage-N-done` (он же становится базой для Захода 1 следующего этапа) и статус меняется на ✅.

**Почему skill, а не отдельный кастомный subagent**: `code-review`/`security-review` — уже встроенные в Claude Code инструменты, которые сами по себе на высоких уровнях строгости используют субагентов внутри. `stage-review` — это просто обёртка-ритуал (два захода, нарастающий скоуп, цикл «нашли → почини → перепроверь»), а не переизобретение bug-hunting логики. Ревью идёт в основной сессии, а не в изолированном subagent'е — так пользователь видит прогресс и получает объяснения по ходу.

**Опциональная эскалация**: для особо чувствительных этапов (например, Этап 4 — авторизация) можно вручную запустить `/code-review ultra` — многоагентное облачное ревью с перекрёстной проверкой находок. Платно, запускается пользователем напрямую, `stage-review` его не вызывает сам.

Кастомный subagent `security-reviewer` (заводится на Этапе 4) не заменяет этот процесс — он дополнительная специализированная проверка конкретно auth-кода поверх общего `stage-review`.
