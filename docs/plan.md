...
1. **Заход 1 (узкий скоуп)** — только диф нового этапа (`git diff stage-(N-1)-done..HEAD`), `code-review` на уровне `high` + `security-review`. Находки чинятся, заход повторяется до чистого результата.
2. **Заход 2 (широкий скоуп)** — весь накопленный код проекта целиком (`apps/api/src apps/web/src packages/`), `code-review` на уровне `xhigh` + `security-review`. Ловит интеграционные проблемы между этапами, которые не видны в узком диффе.
