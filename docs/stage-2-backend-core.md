...
11. Прогнать `/stage-review 2` (см. «Процесс ревью после каждого этапа» в `docs/plan.md`) — только после чистого результата статус меняется на ✅. Обязательно выполнить полную синхронизацию документации («Обязательное обновление документации после этапа» в `docs/plan.md`) — не только таблица в `docs/plan.md`, но и раздел о том, как реализация шла по факту в этом файле, если были отклонения от плана.
