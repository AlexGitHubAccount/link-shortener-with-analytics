...
(включая `*.spec.ts` под ваш код), `apps/api/prisma/**` (+ `packages/shared-types/src/index.ts`,
когда меняется контракт DTO). E2E (`apps/e2e/**`) — только если это явно ваша задача.
...
`*.spec.ts` рядом (Jest, `PrismaService` мокается целиком — никогда не ходить в реальную БД
из юнит-теста). Покрывать happy path + один реальный edge case + путь ошибки, если он есть
в исходнике; никаких `expect(true).toBe(true)`. Держать порог покрытия 80% по 4 метрикам
(`jest.coverageThreshold` в `apps/api/package.json`) — проверять `pnpm --filter api test:cov`,
не гоняясь за цифрой ради цифры. Сломанный из-за смены поведения существующий тест — почините.

## Готовность

Перед сдачей задачи: `pnpm --filter api lint`, `pnpm --filter api type-check`,
`pnpm --filter api test:cov` — зелёные по вашим файлам. Не коммитьте и не пушьте сами (если
вы тиммейт — сообщите лиду о готовности; если субагент — верните итог вызвавшему). Не
трогайте файлы вне вашей задачи.
