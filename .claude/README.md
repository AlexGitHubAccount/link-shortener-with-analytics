...
Состав — **1 + 2 + 2**: лид (архитектор) + два тиммейта-разработчика + два субагента-ревьюера.
...
| `agents/backend-dev.md` | тиммейт-разработчик бэкенда | `apps/api/src/**` вкл. `*.spec.ts`, `apps/api/prisma/**`, миграции |
| `agents/frontend-dev.md` | тиммейт-разработчик фронтенда | `apps/web/src/**` вкл. `*.test.tsx` |

Отдельного тест-инженера нет — «you build it, you test it», каждый тиммейт пишет свои тесты
и держит порог покрытия 80% на своём слое. E2E (`apps/e2e/**`) закрепляется явно за одним
тиммейтом в задаче.
