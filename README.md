...
- **Subagents** (Explore, Plan, general-purpose, кастомные ревьюеры `code-reviewer`/`devsecops-reviewer` и разработчики `backend-dev`/`frontend-dev`/`test-engineer`)
- **Agent Teams** (экспериментальные): `/feature <описание>` — состав 1 + 3 + 2: ведущий (он же архитектор) + три тиммейта (`backend-dev`/`frontend-dev` — продакшен-код, `test-engineer` — все тесты) + два ревьюера без имени (`code-reviewer` всегда + `devsecops-reviewer` на auth-/инфра-поверхности). Ведущий принимает архитектурные решения, раскидывает задачи, координирует, гоняет ревью и push
