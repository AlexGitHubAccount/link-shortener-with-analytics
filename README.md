...
- **Subagents** (Explore, Plan, general-purpose, кастомные `qa-engineer` (ревью + E2E) / `devsecops-reviewer` и разработчики `backend-dev`/`frontend-dev`)
- **Agent Teams** (экспериментальные): `/feature <описание>` — состав 1 + 2 + 2 как в современной продуктовой команде: ведущий (он же архитектор) + два разработчика (`backend-dev`/`frontend-dev`, каждый пишет свой код и свои юнит-тесты) + `qa-engineer` (ревьюит каждое изменение, владеет сквозным E2E) + `devsecops-reviewer` (security/devops-проход по условию). Ведущий принимает архитектурные решения, раскидывает задачи, координирует, гоняет ревью и push
